...
const TrelloSprint: React.FC<TrelloSprintProps> = ({ setSelectedSprint }) => {
    ...
    useEffect(() => {
      const fetchSprints = async () => {
        try {
            const response = await axios.get('https://localhost:7243/api/Trello/dbsprints');
            console.log("Fetched sprints: ", response.data); // Added this line
    ...
    const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (selectedSprint) {
        ...
        const endDate = new Date(startDate.getTime() + (5 - 1) * 24 * 60 * 60 * 1000); // 5 days later
    
        console.log("Start date: ", formatDate(startDate), "End date: ", formatDate(endDate));
        setSelectedSprint({ startDate: formatDate(startDate), endDate: formatDate(endDate) });
      }
    };
